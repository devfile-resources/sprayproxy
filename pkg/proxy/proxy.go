...
func (p *SprayProxy) HandleProxy(c *gin.Context) {
	handleProxyCommon(p, c, false)
}

func (p *SprayProxy) HandleProxyEndpoint(c *gin.Context) {
	handleProxyCommon(p, c, true)
}
...
func handleProxyCommon(p *SprayProxy, c *gin.Context, nonRootEndpoint bool) {
	...
	for _, backend := range p.backends() {
		...
		if nonRootEndpoint {
			newURL.Path = strings.TrimPrefix(newURL.Path, "/proxy")
		}
